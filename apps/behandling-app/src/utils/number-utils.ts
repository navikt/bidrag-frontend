export const roundDown = (num: number) => {
    const numInt = parseInt(num.toString(), 10);
    if (numInt < 10) {
        return numInt;
    }
...
export const formatterBeløpForBeregning = (beløp: number | string | undefined, _visSymbol = false): string => {
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export const formatterBeløp = (beløp: number | string | undefined, _visSymbol = false): string => {
    return (beløp ?? 0).toLocaleString("nb-NO", {
        // style: visSymbol ? "currency" : undefined,
        currency: "NOK",
        minimumFractionDigits: 0,
