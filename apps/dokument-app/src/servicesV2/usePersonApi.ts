...
import type { EnhetDto } from "../api/BidragOrganisasjonApi";
import type { PersonAdresseDto, PersonDto } from "../api/BidragPersonApi";
import type { SamhandlerDto } from "../api/BidragSamhandler";
import { useAppContext } from "../store/AppContext";
import { HTTPStatus } from "../types/enum/HttpStatus";
import type { Person } from "../types/person";
import { BIDRAG_ORGANISASJON_API, PERSON_API, SAMHANDLER_API } from "./api";
...
export const useHentMottakerAdresse = () => {
    ...
    return useSuspenseQuery({
        ...
        queryFn: async () => {
            ...
            return PERSON_API.adresse.hentPersonPostadresse(null, {
                ident: mottakerIdent,
            });
        },
...
export const hentMottakerAdresse = async (mottakerIdent: string): Promise<DistribuerTilAdresse> => {
    if (IdentUtils.isSamhandlerId(mottakerIdent)) {
        return SAMHANDLER_API.samhandler.hentSamhandler(JSON.stringify(mottakerIdent)).then((response) => {
...
export async function hentSamhandler(samhandlerId: string): Promise<SamhandlerDto> {
    const defaultValue: SamhandlerDto = { samhandlerId, navn: "", offentligId: null, offentligIdType: null };

    if (!samhandlerId) return defaultValue;
    try {
        const response = await SAMHANDLER_API.samhandler.hentSamhandler(JSON.stringify(samhandlerId));
...
export async function hentPerson(ident: string): Promise<Person> {
    ...
    try {
        const personResponse = await PERSON_API.informasjon.hentPersonPost({ ident });
