...
describe("maskPathnameForPageId", () => {
    it("maskerer route-parameter for sak og journal", () => {
        expect(maskPathnameForPageId("/sak/1234567/journal/987654321")).toBe(
            "/sak/{saksnummer}/journal/{journalpostId}",
        );
    });

    it("maskerer route-parameter for dokument med dokumentreferanse", () => {
        expect(maskPathnameForPageId("/dokument/123456789/ABC123")).toBe(
