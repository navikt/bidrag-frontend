...
export function SaksumTabellRM({ bidragSak, ident, sak }: Props) {
    ...
    const barnMedRellMottaker =
        roller.filter((rolle) => rolle.reellMottaker?.ident === ident).map((rolle) => rolle.fodselsnummer) ?? [];

    const barn = bidragSak?.barn?.filter((barn) => barnMedRellMottaker.includes(barn.personident ?? "")) ?? [];
