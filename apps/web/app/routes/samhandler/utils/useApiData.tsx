...
import KodeverkApi from "~/routes/samhandler/utils/KodeverkApi.ts";
import {SamhandlerSakerDto} from "@bidrag/api/SakApi";
import {BIDRAG_SAK_API, BIDRAG_SAMHANDLER_API} from "@bidrag/api";
...
export const useHentLandkoder = () => {
    const {data: landkoder} = useSuspenseQuery({
        queryKey: ["landkoder"],
        queryFn: async () => {
            return new KodeverkApi().getLandkoder().then((response) => {
