...
import {SamhandlerDto, SamhandlerSok} from "@bidrag/api/SamhandlerApi";
import {Broadcast, BroadcastNames} from "@bidrag/common";
  ...
  return (
    <VStack gap="space-4" align="start">
      {data.saksnummere.map((saksnummer) => (
        <DsLink
          variant="action"
          href={`${'unleashClient.url.bisysUrl'}/Sak.do?saksnr=${saksnummer}`}//TODO NGHI
          target="_blank"
        >
...
const ExpandableContent = memo(
  ({samhandler, index}: { samhandler: Samhandler; index: number }) => {
    ...
    const background = evenRow ? "surface-subtle" : "bg-default";
    return (
      <HGrid gap="space-4">
...
export default function SamhandlerSøk() {
  ...
  return (
    <div className="m-auto max-w-[1272px] min-[1440px]:max-w-[1920px] p-6 grid gap-4">
