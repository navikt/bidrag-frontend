...
import { EnhetDto, HentEnhetRequest } from "@bidrag/api/OrganisasjonApi";
import { ForelderBarnRelasjonDto, MotpartBarnRelasjonDto, PersonDto, PersonRequest } from "@bidrag/api/PersonApi";
import {
    BidragssakDto,
    FogdhistorikkDto,
    ...
    return useSuspenseQuery({
        ...
        queryFn: async () => {
            try {
                ...
                SecureLoggerService.error("Kunne ikke hente informasjon om saksbehandler kan opprette sak uten BM", e);
                return { data: false };
            }
        },
        ...
        queryFn: async () => {
            try {
                ...
                if (status === 403 || status === 401) {
                    ...
                    await SecureLoggerService.warn("Feil ved tilgangssjekk sak uten BM - antar ingen tilgang", e);
                }
    ...
    return useMutation<string, AxiosError<string> | TilgangsFeilError, OpprettSakRequest>({
        ...
        mutationFn: async (request: OpprettSakRequest) => {
            try {
                ...
                await SecureLoggerService.error(`Kunne ikke opprette sak for request ${JSON.stringify(request)}`, e);
                throw e;
            }
        },
    });
    ...
    return useQuery<BidragssakDto[], AxiosError | TilgangsFeilError>({
        ...
        queryFn: async () => {
            try {
                ...
                await SecureLoggerService.error(`Kunne ikke hente sak for person ${ident}`, e);
                throw e;
            }
        },
        ...
        mutationFn: async ({ ident }: { ident: string }): Promise<ISamhandlerPersonInfo> => {
            ...
            try {
                if (IdentUtils.isSamhandlerId(ident) && sjekkSamhandler) {
                    ...
                    const response = await BIDRAG_PERSON_API.informasjon.hentPersonPost({ ident });
...
export function useHentPersonMotpartBarnRelasjonSuspense(request: PersonRequest | null, enabled: boolean = true) {
    return useSuspenseQuery<MotpartBarnRelasjonDto, AxiosError | TilgangsFeilError>({
    ...
    return useQuery<FogdhistorikkDto[], AxiosError | TilgangsFeilError>({
        ...
        queryFn: async () => {
            try {
                ...
                await SecureLoggerService.error(`Kunne ikke hente fogdhistorikk for saksnummer ${saksnummer}`, e);
                throw e;
            }
        },
