...
function SaksrolleroversiktProvider({ children }: PropsWithChildren) {
    ...
    const velgPerson = useCallback((person: PersonDto) => {
        nesteVersjon();
    ...
    const velgSakstype = useCallback((type: Sakstype) => {
        nesteVersjon();
    ...
    const velgKategori = useCallback((kategori: Sakskategori) => {
        nesteVersjon();
        valgtPersonRef.current = null;
    ...
    const velgRolle = useCallback((rolle: PartISaken["rolle"]) => {
