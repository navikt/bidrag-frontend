...
import {InternalRolleDto, Kategori, SakStatus} from "../types/api/SakTypes";
import {Journalpost} from "../types/journalpost";
import {erSamhandlerId, Person} from "../types/person";
...
import {BidragssakDto, Rolletype} from "@bidrag/api/SakApi";
import {EnhetDto} from "@bidrag/api/OrganisasjonApi";
import {OrganisasjonQueryFunctions} from "~/routes/dokument/utils/useOrganisasjonApi.ts";
...
async function hentSakerEnhetsInfo(saker: Sak[], queryClient: QueryClient): Promise<EnhetDto[]> {
    const eierfogdList = Array.from(
        new Set(saker.filter((sak) => sak.eierfogd !== undefined || sak.eierfogd !== "null").map((sak) => sak.eierfogd))
    );
