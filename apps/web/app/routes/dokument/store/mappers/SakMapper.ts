...
import { JournalforendeEnhetDto } from "../../api/BidragOrganisasjonApi";
import { InternalRolleDto } from "../../types/api/SakTypes";
    ...
    static mapSakerForPerson(
        ...
        return _.uniqBy(saker, "saksnummer").map((currentSak) => {
            const updatedSak: Sak = {
                ...currentSak,
                ferdigRegistrert: this.isFerdigregistrert(currentSak),
                eierfogd: currentSak.eierfogd === "null" ? undefined : currentSak.eierfogd,
                enhetInformasjon: this.findEnhetsNavn(currentSak.eierfogd, enhetsInfo),
                rolle: this.findRolleInRoller(personIdent, currentSak),
                tilknyttetJournalpost: saksnummerJournalpost.includes(currentSak.saksnummer),
            };
            updatedSak.motsattRolle = this.findMotsattPartInSakForEnkelSak(personIdent, updatedSak);
            return updatedSak;
        });
    }
