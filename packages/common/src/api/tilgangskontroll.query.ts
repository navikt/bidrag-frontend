...
export function sjekkTilgangSak(saksnummer?: string) {
    return queryOptions({
        ...
        queryFn: () =>
            withQueryErrorHandlingV2("sjekkTilgangSakV2", async () => {
                const { data } = await BIDRAG_TILGANGSKONTROLL_API.v2.sjekkTilgangSakV2({
...
export function sjekkTilgangPerson(personident: string) {
    return queryOptions({
        ...
        queryFn: () =>
            withQueryErrorHandlingV2("sjekkTilgangPerson", async () => {
